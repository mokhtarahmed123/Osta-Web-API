        ...
        protected override async Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            consumer.ReceivedAsync += async (model, eventArgs) =>
            {
                try
                {
                    ...
                    var notification =
                        JsonSerializer.Deserialize<NotificationMessage>(message);

                    if (notification is null)
                        throw new Exception("Invalid notification message.");

                    var email = new Emaildto(notification.RecipientEmail, notification.Message, "OSTA Notification");
            ...
            Console.WriteLine("Notification Consumer is waiting for messages...");
